...
public class Inflector {
	...
	public static String camelCase(final String name) {
		...
		for (final String part : name.split("_")) {
			builder.append(Character.toTitleCase(part.charAt(0))).append(
					part.substring(1));
		}
	...
	private String replaceWithFirstRule(final String word,
			final List<RuleAndReplacement> ruleAndReplacements) {
		for (final RuleAndReplacement rar : ruleAndReplacements) {
			...
			final Matcher matcher = Pattern.compile(rule,
