...
class Order(order : jpa.shop.Order) extends Delegate(order) {
  ...
  def updateVolume(productOrder : jpa.shop.ProductOrder, v : Int) : Boolean = {
    if (productOrder.getVolume != v) {
      ...
      productOrder.setPrice(v * ShopModel.shop.get.productsById(productOrder.getProduct().getId().longValue()).propertiesByName("Price").pvalue.getMoneyValue.doubleValue)
      return true
    }
    return false
  }
  ...
  def addProduct(product : Product, volume : Int) = {
    ...
    delegate.getProductOrders find((po) =>
      ShopModel.shop.productsById(po.getProduct().getId().longValue()).propertiesByName("ArticleNumber").pvalue.getStringValue
          == arn) match {
        case None =>
